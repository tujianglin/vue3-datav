 ...
 * @param scale 缩放比例
 */
function calcResizeForCorner(
  direction: Direction,
  attr: ComponentAttr,
  curPositon: IPoint,
  symmetricPoint: IPoint,
  scale: number,
  pos: Partial<ComponentAttr>,
) {
...
function calcResizeForEdge(
  ...
  if (dir === 't' || dir === 'b') {
    const newCornerPoint = calcRotatedPoint(
      { x: startPoint.x, y: newDiagonalPoint.y },
      startPoint,
      attr.deg,
    );
    // 勾股定理
    const height = Math.sqrt(
      (newCornerPoint.x - symmetricPoint.x) ** 2 + (newCornerPoint.y - symmetricPoint.y) ** 2,
    );
    if (height > 0) {
      const center = {
  ...
  if (dir === 'l' || dir === 'r') {
    const newCornerPoint = calcRotatedPoint(
      { x: newDiagonalPoint.x, y: startPoint.y },
      startPoint,
      attr.deg,
    );
...
export const handleZoom = (
  ...
  const move = (e: MouseEvent) => {
    ...
    if (pos.w != undefined) {
      (com.scaling as any).w = Math.max(10, pos.w);
    }

    if (pos.h != undefined) {
      (com.scaling as any).h = Math.max(10, pos.h);
    }
