<script lang="tsx">
  ...
  export default defineComponent({
    setup() {
      ...
      const hRulerWpRef = ref();
      const vRulerWpRef = ref();
            ...
            hRuler.value = new RulerBuilder(hRulerWpRef.value, {
              ...
              coorChange: (action, nCoor, oCoor) => {
                if (action === 'add') {
                  ...
                  guideLine.h = guideLine.h.filter((i) => i !== oCoor);
                  guideLine.h.push(nCoor);
                } else if (action === 'delete') {
                  guideLine.h = guideLine.h.filter((i) => i !== nCoor);
                }
              },
            });
      ...
      return () => (
        <div class="canvas-ruler-wp">
          <div
            ...
            style={{ transform: `translateX(-${hScroll.value}px)` }}
          ></div>
          <div
            ref={vRulerWpRef}
            class="ruler-wrapper v-container"
            style={{ transform: `rotate(90deg) translateX(-${vScroll.value}px)` }}
          ></div>
